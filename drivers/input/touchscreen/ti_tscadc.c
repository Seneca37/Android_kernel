...
#define TSCADC_REG_SYSCONFIG		0x010
...
#define TSCADC_CNTRLREG_POWERENB		BIT(4)
...
static int ts_dev_thread(void* _ts_dev)
{
    unsigned int idleconfig ,ctl,sysconfig,adcstat,ADC_TSC_CLKCTRL;

	struct tscadc		*ts_dev = (struct tscadc *)_ts_dev;
...
//	idleconfig = tscadc_readl(ts_dev, TSCADC_REG_IDLECONFIG);
//	printk("%s() idleconfig  0x%x\n\n", __func__,idleconfig);
#if 0
	ctl = tscadc_readl(ts_dev, TSCADC_REG_CTRL);
	ctl |= TSCADC_CNTRLREG_POWERENB;
	ctl &= ~(TSCADC_CNTRLREG_TSCENB);
	tscadc_writel(ts_dev, TSCADC_REG_CTRL, ctl);
...
//	sysconfig = tscadc_readl(ts_dev, TSCADC_REG_SYSCONFIG);
//	printk("%s() sysconfig  0x%x\n\n", __func__,TSCADC_REG_SYSCONFIG);
	//TSCADC_REG_SYSCONFIG

	adcstat = tscadc_readl(ts_dev, TSCADC_REG_ADCFSM);
	printk("%s() adcstat  0x%x\n\n", __func__,adcstat);

//	ADC_TSC_CLKCTRL = readl(AM33XX_L4_WK_IO_ADDRESS(AM33XX_PRCM_BASE)+0x400+0xbc);
//	printk("%s() ADC_TSC_CLKCTRL  0x%x\n\n", __func__,ADC_TSC_CLKCTRL);
...
static	int __devinit tscadc_probe(struct platform_device *pdev)
{
  ...
  poll_task = kthread_create(ts_dev_thread, ts_dev, "ts_dev-gpio-poll-thread");
  if (IS_ERR(poll_task)) {
      printk("\ts_dev-gpio-poll-thread create error: %p\n", poll_task);
  		err = PTR_ERR(poll_task);
			poll_task = NULL;
      kfree(ts_dev);
     return err;
   }
