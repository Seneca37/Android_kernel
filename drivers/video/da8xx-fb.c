...
#if defined (LCD_070INC_3UART) ||defined (LCD_070INC)

#include "logo-800.h"

#define PLCD_WIDTH	800
#define PLCD_HEIGHT	480
#define PBYTE_PER_PIXEL  4  //Every pixel use 4  bytes
#define ULOGO_WIDTH  800    //Refer to Logo image size
#define ULOGO_HEIGHT 480    //Refer to Logo image size
...
#elif defined (LCD_035TINC) ||defined (LCD_035INC)
#include "logo-320.h"

#define PLCD_WIDTH	320
#define PLCD_HEIGHT	240
#define PBYTE_PER_PIXEL  4  //Every pixel use 4  bytes
#define ULOGO_WIDTH  320    //Refer to Logo image size
#define ULOGO_HEIGHT 240    //Refer to Logo image size

#elif LCD_040INC
#include "logo-320inv.h"
...
#define PLCD_WIDTH	480
#define PLCD_HEIGHT	272
#define PBYTE_PER_PIXEL  4  //Every pixel use 4  bytes
#define ULOGO_WIDTH  480    //Refer to Logo image size
#define ULOGO_HEIGHT 272    //Refer to Logo image size
...
#define PLCD_WIDTH	640
#define PLCD_HEIGHT	480
#define PBYTE_PER_PIXEL  4  //Every pixel use 4  bytes
#define ULOGO_WIDTH  640    //Refer to Logo image size
#define ULOGO_HEIGHT 480    //Refer to Logo image size
#endif
...
static void init_logo(unsigned char * framebuffer)
{
//	printk("%s,%d: init kernel logo!\n",__func__,__LINE__);
	unsigned int i = 0;
	unsigned int j = 0;
	unsigned int k = 0;
	unsigned int wd,ht;
...
#ifdef DBN
#else
	unsigned int logo_start_offset = ((PLCD_WIDTH-ULOGO_WIDTH)/2)*PBYTE_PER_PIXEL;
	unsigned int lcd_line_length   =  (PLCD_WIDTH)*PBYTE_PER_PIXEL; 
	...
	wd=PLCD_WIDTH;
	ht=PLCD_HEIGHT;
	printk("%s,%d: init kernel logo!wd=%d,ht=%d\n",__func__,__LINE__,wd,ht);
	
	ImgBuf = (unsigned char *) framebuffer; 
	if(4 == PBYTE_PER_PIXEL){ //32bit mode,use ARGB8888
		memset((unsigned char*) ImgBuf, 0, PLCD_WIDTH*PLCD_HEIGHT*PBYTE_PER_PIXEL);//Keep Logo on the center of screen
	    for(i=0; i<ULOGO_HEIGHT; i++){
		    Buf = (unsigned char*) (ImgBuf +(i*lcd_line_length)+logo_start_offset);
		    for(j=0; j<(ULOGO_WIDTH*PBYTE_PER_PIXEL); j++){
			    *Buf++ = bootlogo[k++];
			}
		}
	}
...
static int __devinit fb_probe(struct platform_device *device)
{
	...
	if (!par->vram_virt) {
	    ...
	    printk("map_video_memory(%p): phys: 0x%x , size:0x%lx , virt:0x%lx\n",par,par->vram_phys,par->vram_size,(unsigned long )par->vram_virt);
	}
    init_logo(par->vram_virt);
