...
#define SAMKOON_LCD_BL GPIO_TO_PIN(0, 7)
#define SAMKOON_LCD_BL1 GPIO_TO_PIN(3,19)

#ifdef LCD_ALLINONE_050INC
#define SAMKOON_UART0 GPIO_TO_PIN(0,6)
#define SAMKOON_UART1 GPIO_TO_PIN(3,21)
#else
#define SAMKOON_UART0 GPIO_TO_PIN(0,12)
#define SAMKOON_UART1 GPIO_TO_PIN(0,13)
#endif

#define SAMKOON_UART3 GPIO_TO_PIN(2,23)

#define SAMKOON_LED 	GPIO_TO_PIN(3,20)

//#define SAMKOON_TXD0 GPIO_TO_PIN(1,11)
//#define SAMKOON_TXD1 GPIO_TO_PIN(0,15)
#define SAMKOON_TXD GPIO_TO_PIN(1,8)
...
static const struct display_panel inl_disp_panel = {
	WVGA,
	32,
	32,
	COLOR_ACTIVE,
...
static const struct display_panel kd_disp_panel = {
	QVGA,
	32,
	32,
	COLOR_ACTIVE,
...
#define AM335X_PWM_PERIOD_NANO_SECONDS    (1000 * 66)
// (5000 * 10*100)
...
static struct lcd_ctrl_config Inl_lcd_cfg = {
	&inl_disp_panel,
	.ac_bias		= 255,
	.ac_bias_intrpt		= 0,
	.dma_burst_sz		= 16,
...
static struct lcd_ctrl_config kd_lcd_cfg = {
	&kd_disp_panel,
	.ac_bias		= 255,
	.ac_bias_intrpt		= 0,
	.dma_burst_sz		= 16,
...
struct da8xx_lcdc_platform_data INL_AT070TN92_pdata = {
	.manu_name		= "Innolux",
	.controller_data	= &Inl_lcd_cfg,
	.type			= "INL_AT070TN92",
};

struct da8xx_lcdc_platform_data INL_AT102TN03_pdata = {
	.manu_name		= "Innolux",
	.controller_data	= &Inl_lcd_cfg,
	.type			= "INL_AT102TN03",
};

struct da8xx_lcdc_platform_data KD35G6_54NT_pdata = {
	.manu_name		= "Kd35g6",
	.controller_data	= &kd_lcd_cfg,
	.type			= "KD35G6_54NT",
};

struct da8xx_lcdc_platform_data KD40G1_54NC_pdata = {
	.manu_name		= "Kd40g1",
	.controller_data	= &kd_lcd_cfg,
	.type			= "KD40G1_54NC",
};

struct da8xx_lcdc_platform_data KD43G18_40NB_pdata = {
	.manu_name		= "Kd43g18",
	.controller_data	= &kd_lcd_cfg,
	.type			= "KD43G18_40NB",
};
...
struct da8xx_lcdc_platform_data KD50G23_40NB_pdata = {
	.manu_name		= "Kd50g23",
	.controller_data	= &kd_lcd_cfg,
	.type			= "KD50G23_40NB",
};

struct da8xx_lcdc_platform_data INL_AT056TN52_pdata = {
	.manu_name		= "Innolux",
	.controller_data	= &Inl_lcd_cfg,
	.type			= "INL_AT056TN52",
};
...
static struct omap2_hsmmc_info am335x_mmc[] __initdata = {
	{
		...
		.gpio_cd        =GPIO_TO_PIN(3, 14),// GPIO_TO_PIN(0, 6),
...
#ifdef LCD_ALLINONE_050INC
	{"spi0_cs1.gpio0_6", OMAP_MUX_MODE7 |AM33XX_PIN_OUTPUT},
	{"mcasp0_ahclkx.gpio3_21", OMAP_MUX_MODE7 | AM33XX_PIN_OUTPUT},
#endif
	{"mcasp0_fsx.gpio3_15", OMAP_MUX_MODE7 |AM33XX_PIN_INPUT},
#ifndef LCD_ALLINONE_050INC
...
/* Module pin mux for voltage detect */
static struct pinmux_config voltage_detect_pin_mux[] = {
	{"xdma_event_intr0.gpio0_19", OMAP_MUX_MODE7 |AM33XX_PIN_INPUT},
	{NULL, 0},
};

/* Module pin mux for vibrator */
static struct pinmux_config vibrator_pin_mux[] = {
#ifdef LCD_ALLINONE_050INC
...
static void lcdc_init(int evm_id, int profile)
{
	...
	gpio_request(SAMKOON_LCD_BL1, "SAMKOON_LCD_BL1");
	gpio_direction_output(SAMKOON_LCD_BL1, 1);
	gpio_request(SAMKOON_LCD_BL, "SAMKOON_LCD_BL");
	gpio_direction_output(SAMKOON_LCD_BL, 1);
	printk("enable gpio3_19,gpio0_7\n");

//	while(1);

	gpio_request(SAMKOON_LED, "SAMKOON_LED");
	gpio_direction_output(SAMKOON_LED, 0);
	gpio_request(SAMKOON_UART0, "SAMKOON_UART0");
	gpio_direction_output(SAMKOON_UART0, 0);
	gpio_request(SAMKOON_UART1, "SAMKOON_UART1");
	gpio_direction_output(SAMKOON_UART1, 0);

	gpio_request(SAMKOON_TXD, "SAMKOON_TXD");
//	gpio_request(SAMKOON_TXD1, "SAMKOON_TXD1");
	
#if defined(LCD_070INC_3UART)
	printk("LCD_070INC_3UART\n");
	gpio_request(SAMKOON_UART3, "SAMKOON_UART3");
	gpio_direction_output(SAMKOON_UART3, 0);
#endif

#if defined (LCD_035INC) || defined (LCD_035TINC)
	if (am33xx_register_lcdc(&KD35G6_54NT_pdata))
		printk("Failed to register LCDC device\n");
#elif defined LCD_070INC || defined (LCD_070INC_3UART)
	if (am33xx_register_lcdc(&INL_AT070TN92_pdata))
		printk("Failed to register LCDC device\n");
#elif defined (LCD_102INC)
	if (am33xx_register_lcdc(&INL_AT102TN03_pdata))
		printk("Failed to register LCDC device\n");
#elif defined (LCD_040INC)
	KD40_init();
	if (am33xx_register_lcdc(&KD40G1_54NC_pdata))
		printk("Failed to register LCDC device\n");
#elif defined (LCD_043INC) ||defined(LCD_043INC_2UART)
	if (am33xx_register_lcdc(&KD43G18_40NB_pdata))
		printk("Failed to register LCDC device\n");

#elif defined (LCD_050INC) ||defined(LCD_ALLINONE_050INC)
	if (am33xx_register_lcdc(&KD50G23_40NB_pdata))
		printk("Failed to register LCDC device\n");

#elif defined (LCD_057INC)
	if (am33xx_register_lcdc(&INL_AT056TN52_pdata))
		printk("Failed to register LCDC device\n");
#endif
...
//#define RMII_RESET GPIO_TO_PIN(3, 21)
#define IPA_PHY_ID		0x02430c54
#define IPA_PHY_MASK		0xfffffffd
#define IP101A_APS_ON			2   /* IP101A APS Mode bit */

static int am335x_ipa101_phy_fixup(struct phy_device *phydev)
{
	 int err,val;
...
//	printk("register 0 0x%x\n",val);
	
	val = phy_read(phydev, 1);
//	printk("register 1 0x%x\n",val);

	val = phy_read(phydev, 2);
//	printk("register 2 0x%x\n",val);

	val = phy_read(phydev, 3);
//	printk("register 3 0x%x\n",val);

	val = phy_read(phydev, 4);
//	printk("register 4 0x%x\n",val);

	val = phy_read(phydev, 5);
//	printk("register 5 0x%x\n",val);

	val = phy_read(phydev, 6);
//	printk("register 6 0x%x\n",val);
...
//	printk("register 16 0x%x\n",val);

	val = phy_read(phydev, 17);
//	val = val&0x7fff;
//	phy_write(phydev, 17, val );
//	val = phy_read(phydev, 17);
//	printk("register 17 0x%x\n",val);

	val = phy_read(phydev, 18);
//	printk("register 18 0x%x\n",val);

	val = phy_read(phydev, 30);
//	printk("register 30 0x%x\n",val);
...
static void rmii1_init(int evm_id, int profile)
{
	...
	phy_register_fixup_for_uid(IPA_PHY_ID, IPA_PHY_MASK,
					am335x_ipa101_phy_fixup);
...
#ifdef LCD_ALLINONE_050INC
#define VIBRATOR_PIN  GPIO_TO_PIN(3, 16)
#else
#define VIBRATOR_PIN  GPIO_TO_PIN(0, 6)
#endif
...
{
	setup_pin_mux(vibrator_pin_mux);
	printk("register vibrator,c15 gpoi\n");
	platform_device_register(&android_vibrator_device);
	return;
}

#if 0
struct platform_device android_voltage_detect_device = {
	.name		= "switch-gpio-voltage-detect",
	.id		= -1,
};
#else
#include <linux/switch.h>
#define VOLTAGE_IRQ_GPIO		GPIO_TO_PIN(0, 19) //GPIO_TO_PIN(3, 17) //GPIO_TO_PIN(0, 19)

static struct gpio_switch_platform_data android_voltage_switch_data = {
       .name = "samkoon",
       .gpio = VOLTAGE_IRQ_GPIO,      
};

struct platform_device android_voltage_detect_device = {
	.name		= "switch-gpio",
	.id		= -1,
	 .dev = {.platform_data    = &android_voltage_switch_data,}
};

struct platform_device android_voltage_detect_device1 = {
	.name		= "switch-gpio-voltage-detect",
	.id		= -1,
};
#endif
/* setup voltage detect */
static void voltage_detect_init()
{
	setup_pin_mux(voltage_detect_pin_mux);
	printk("register voltage detect\n");
	platform_device_register(&android_voltage_detect_device);
	platform_device_register(&android_voltage_detect_device1);
	return;
}
...
/*
 * SPI touch panel control
 *
 */
 #define AM335XEVM_TS_IRQ_GPIO		GPIO_TO_PIN(3, 15)

static int sx8652_pendown_state(void)
{
	return !gpio_get_value(AM335XEVM_TS_IRQ_GPIO);	/* Touchscreen PENIRQ */
}
...
static struct spi_board_info am335x_spi0_touchpanel_info[] = {
	{
		.modalias	= "sx8652",
		.chip_select	= 0,
		...
		.irq		= OMAP_GPIO_IRQ(AM335XEVM_TS_IRQ_GPIO),
...
static void evm_nand_init(int evm_id, int profile)
{
	...
	if (!pdata)
		return;
	pdata->ecc_opt =OMAP_ECC_HAMMING_CODE_DEFAULT;//OMAP_ECC_HAMMING_CODE_DEFAULT;//saveenvOMAP_ECC_BCH8_CODE_HW;
	pdata->elm_used = true;
...
#ifdef LCD_ALLINONE_050INC
#define SXIRQ_PIN  GPIO_TO_PIN(0, 13)
#define SXRST_PIN  GPIO_TO_PIN(0, 12)
#else
#define SXIRQ_PIN  GPIO_TO_PIN(0, 4)
#define SXRST_PIN  GPIO_TO_PIN(0, 5)
#endif
static int sx_get_pendown_state(void)
{
	return !gpio_get_value(SXIRQ_PIN);	/* Touchscreen PENIRQ */
}
...
struct sx8650_platform_data sx8650_data = {
	.model = 8650,
	.y_plate_ohms = 250,
...
static int mxt224_get_hw_status(void)
{
	return gpio_get_value(SXIRQ_PIN);	/* Touchscreen PENIRQ */
}

struct multek_ts_platform_data mxt224_data = {
	.hw_status = mxt224_get_hw_status,
};
...
#define CS_PIN  GPIO_TO_PIN(0, 14)
...
void  KD40_init()
{
	...
	Write_com_reg(0x0005,0xbc44);
	Write_com_reg(0x0003,0x3532);	 
	Write_com_reg(0x001e,0x00f5);
	Write_com_reg(0x000e,0x1080);	   

	Write_com_reg(0x0030,0x0000);
	Write_com_reg(0x0031,0x0407);	 
	Write_com_reg(0x0032,0x0203);
	Write_com_reg(0x0033,0x0000);	



	Write_com_reg(0x0034,0x0505);
	Write_com_reg(0x0035,0x0003);	 
	Write_com_reg(0x0036,0x0707);
	Write_com_reg(0x0037,0x0000);

	Write_com_reg(0x003a,0x0c0b);
	Write_com_reg(0x003b,0x1304);		 
...
#else
	gpio_request(SCK_PIN, "SCK_PIN");
	gpio_request(MOSI_PIN, "MOSI_PIN");
	gpio_request(MISO_PIN, "MISO_PIN");
	gpio_request(CS_PIN, "CS_PIN");
	gpio_request(IRQ_PIN, "IRQ_PIN");
	gpio_request(SPIRESET_PIN, "SPIRESET_PIN");


	spi_register_board_info(am335x_spi0_touchpanel_info,
			ARRAY_SIZE(am335x_spi0_touchpanel_info));
#endif
	return;
}
...
//								PROFILE_5)},
#if defined (LCD_070INC) ||defined(LCD_043INC_2UART)||defined (LCD_070INC_3UART) ||defined (LCD_ALLINONE_050INC) ||(LCD_102INC) ||defined (LCD_057INC)
	{uart1_init,	DEV_ON_BASEBOARD, (PROFILE_0 | PROFILE_3 |
								PROFILE_5)},
#endif
...
#ifdef LCD_070INC_3UART
	{uart2_init,	DEV_ON_DGHTR_BRD, PROFILE_ALL},
#endif
...
static void am335x_evm_setup(void)
{
	...
	strcpy(config.opt,"SKU#01FFFFFFFFFFFFFFFFFFFFFFFFFF");
	strcpy(config.version,"1.1A50114P160081SKU#01FFFFFFFFFFFFFFFFFFFFFFFFFF");
	strcpy(config.name,"A33515BB1.1A50114P160081SKU#01FFFFFFFFFFFFFFFFFFFFFFFFFF");
	strcpy(config.serial,"50114P160081SKU#01FFFFFFFFFFFFFFFFFFFFFFFFFF");
	printk("spl_board_init config.header=0x%x\n",config.header);
	printk("spl_board_init config.opt=%s\n",config.opt);
	printk("spl_board_init config.version=%s\n",config.version);
	printk("spl_board_init config.name=%s\n",config.name);
	printk("spl_board_init config.serial=%s\n",config.serial);
	pr_info("Detected a daughter card on AM335x EVM..");
/*
	am335x_mac_addr[0][0]=0xd494a1326bdc;
	am335x_mac_addr[1][0]=0xd494a1326bdc;
	am335x_mac_addr[2][0]=0xd494a1326bdc;
	am33xx_cpsw_macidfillup(&am335x_mac_addr[0][0],
				&am335x_mac_addr[1][0]);
*/
	daughter_brd_detected = true;
...
#if defined(CONFIG_TOUCHSCREEN_SX8650)	
	{
		I2C_BOARD_INFO("sx8650", 0x48),
		.platform_data = &sx8650_data,	  	
		.type		= "sx8650",		
		.irq		= OMAP_GPIO_IRQ(SXIRQ_PIN),
	},
#elif defined(CONFIG_TOUCHSCREEN_ATMEL_MXT224E)
	{
		I2C_BOARD_INFO("multek_ts",  0x4B), //0x4B 0x4A
...
static struct omap_musb_board_data musb_board_data = {
	 ...
	 */
//	.mode           = (MUSB_HOST << 4) | MUSB_OTG, //MUSB_PERIPHERAL
...
//	evm_init_cpld();
	printk("am335x_evm_i2c_init 1111\n");

	setup_pin_mux(i2c0_pin_mux);
	gpio_request(SXIRQ_PIN, "SXIRQ_PIN");
	gpio_request(SXRST_PIN, "SXRST_PIN");
...
/*
static struct platform_device am335x_d_can_device = {
	.name		= "d_can",
	.id		= -1,
};
//*/
...
//	platform_device_register(&am335x_d_can_device);
